...
export class EpickService {
  ...
  async createOverrideRequest(data: {
    ...
    // Validate qty based on request type
    if (finalRequestType === 'scan') {
      if (!qty || qty <= 0) {
        throw new AppError('Quantity must be greater than 0 for scan type requests', 400);
      }
    }
  ...
  async requestAllStatusOverride(orderNumber: number, query: any) {
    ...
    // First, find the pending requests that will be updated (to get their IDs)
    const pendingRequests = await OverrideRequest.findAll({
      where: whereCondition,
    });

    // Get IDs of requests that will be updated
    ...
    if (status === 'approved' && requestIds.length > 0) {
      ...
      // Get or create a box for this order (use first box or default) - do this once outside the loop
