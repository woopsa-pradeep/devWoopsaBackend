...
export class EpickService {
  ...
  async acceptOrder(body: IOrderPick, id: number) {
    ...
    for (const confirmation of existingConfirmations) {
      ...
      if (hasOverlap) {
        ...
        // Note: confirmation.pickerUserNumber is INTEGER, EpickUser.userNumber is STRING
        const picker = await EpickUser.findOne({
          where: { userNumber: String(confirmation.pickerUserNumber) },
    ...
    await EpickConfirmation.create({
      orderNumber: body.orderNumber,
      pickerUserNumber: Number(isUserExist.userNumber ?? 0), // Store userNumber (display number)
      pickerUserId: id, // Store user's id (database ID)
  ...
  async OrderCompleted(orderNumber: number, pickerUserId: number) {
    ...
    await OrderHeader.update({
      EpickCompletedOn: fn('GETDATE'),
      EpickStatusFromPicker: '1',
      AssignedToEPickUser: pickerUserNumber
    }, {
      where: {
        Order_Number: orderNumber
      }
    ...
    const currentConfirmation = await EpickConfirmation.findOne({
      where: {
        orderNumber: orderNumber,
        pickerUserNumber: Number(pickerUserNumber), // Use userNumber (convert to number), not userId
        status: 'in_progress'
      }
    });
  ...
  async getCompleteOrder() {
    ...
    const completedOrders = await OrderPick.findAll({
      where: {
        status: { [Op.or]: ['completed', 'ready_for_delivery'] } // Only get completed orders (already picked by epick)
      },
