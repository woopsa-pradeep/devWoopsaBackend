...
export class SalesService {
  ...
  async getPriceBookInventoryItems(query: PaginationOptions & { search?: string; masterSearch?: string }, customerId: number) {
    let {
      page = 1,
      limit = 10,
      salesCategoryId,
  ...
  async getTradeShowItems(query: PaginationOptions & { search?: string, masterSearch?: string }, customerId: number) {
    ...
    const userJurisdiction = await getJurisdiction(Number(customerId));
    ...
    if (state || zip || jurisdiction) {
      const excludeItem = await getCustomerExcludeItemForTradeShow(state as string, zip as string, Number(jurisdiction) as number);
      whereClause.itemNumber = { [Op.notIn]: excludeItem };
    }
  ...
  async setSalesSession(userId: number, customerId: number) {
    ...
    const isTradeShow = await TradeShowRetailer.findOne({ where: { retailerId: Number(customerId) } });
  ...
  async addToTradeShowCart(cartData: AddToCartRequest & { Customer_Number: number }, salesId: number) {
    // Check if item already exists in cart for this customer
    cartData.TotalPrice = Number(cartData.TotalPrice);
    cartData.TotalPriceWithTax = Number(cartData.TotalPriceWithTax);
    cartData.discount = Number(cartData.discount);
    ...
    if (existingCartItem) {
      ...
      // Create new cart item
      const newCartItem = await CustomerCart.create({
        ...cartData, type: 'tradeShow', placedBySalesPerson: true, salesPersonNumber: salesId,
        discount: cartData.discount || 0,
        originalPrice: cartData.originalPrice || 0
      });
      return newCartItem;
